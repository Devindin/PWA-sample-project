...
function DetailsPanel({ selected, formValues, handleChange, onSave, onDelete, fields, title }) {
  if (!selected) {
    return (
      <div className="w-2/3 bg-white dark:bg-gray-800/60 backdrop-blur-lg rounded-xl shadow p-6 flex items-center justify-center">
  ...
  return (
    <div className="w-2/3 bg-white dark:bg-gray-800/60 backdrop-blur-lg rounded-xl shadow p-6 flex flex-col gap-4 overflow-y-auto">
      <h2 className="text-2xl font-bold mb-4">{formValues.name || `New ${title}`}</h2>
